...
AControlledTest::AControlledTest()
{
...
void AControlledTest::BeginPlay()
{
	Super::BeginPlay();
...
bool AControlledTest::StartUDPReceiver(
	...
	UDPReceiver->OnDataReceived().BindUObject(this, &AControlledTest::Recv);
	UDPReceiver->Start();
	return true;
}

void AControlledTest::Recv(const FArrayReaderPtr& ArrayReaderPtr, const FIPv4Endpoint& EndPt)
{
...
void AControlledTest::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
// Called every frame
void AControlledTest::Tick(float DeltaSeconds)
{
...
void AControlledTest::UpdateTexture()
{
...
void AControlledTest::PostActorCreated()
{
...
void AControlledTest::PostLoad()
{
	Super::PostLoad();
	GenerateMesh();
}

void AControlledTest::SetupMeshBuffers()
{
...
void AControlledTest::GeneratePoints()
{
...
void AControlledTest::GenerateMesh()
{
...
void AControlledTest::GenerateGrid(TArray<FVector>& InVertices, TArray<int32>& InTriangles, TArray<FVector2D>& InUV0, FVector2D InSize, int32 InLengthSections, int32 InWidthSections, const TArray<float>& InHeightValues)
{
...
#if WITH_EDITOR
void AControlledTest::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if ((MemberPropertyChanged == GET_MEMBER_NAME_CHECKED(AControlledTest, Size)) || (MemberPropertyChanged == GET_MEMBER_NAME_CHECKED(AControlledTest, ScaleFactor)))
	{
		// Same vert count, so just regen mesh with same buffers
		GenerateMesh();
	}
	else if ((MemberPropertyChanged == GET_MEMBER_NAME_CHECKED(AControlledTest, LengthSections)) || (MemberPropertyChanged == GET_MEMBER_NAME_CHECKED(AControlledTest, WidthSections)))
	{
	...
	//else if ((MemberPropertyChanged == GET_MEMBER_NAME_CHECKED(AControlledTest, Material)))
	//{
	//	MeshComponent->SetMaterial(0, Material);
	//}
}
...
void AControlledTest::UpdateTextureRegions(UTexture2D* Texture, int32 MipIndex, uint32 NumRegions, FUpdateTextureRegion2D* Regions, uint32 SrcPitch, uint32 SrcBpp, uint8* SrcData, bool bFreeData)
{
	if (Texture->Resource)
	{
